...
class minIMU(LIS3MDL, LSM6DS33):
    ...
    def getGyroFil(self):
        """ Returns a vector (list) of gyroscope measurements run through a LPF. """
        gyros = self.getGyroCal()
        self.gyrX = self.LPF*gyros[0] + (1-self.LPF)*self.gyrX
        self.gyrY = self.LPF*gyros[1] + (1-self.LPF)*self.gyrY
        self.gyrZ = self.LPF*gyros[2] + (1-self.LPF)*self.gyrZ
    ...
    def getAccelFil(self):
        """ Returns a vector (list) of accelerometer run measurements through a exponential moving average filter. """
